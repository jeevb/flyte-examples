...
def generate_prompt(instruction: str, input: Optional[str] = None, output: Optional[str] = None) -> str:
...
class TokenizerHelper:
    ...
    def generate_and_tokenize_prompt(self, data_point):
        ...
        if not self.train_on_inputs:
            ...
            tokenized_full_prompt["labels"] = [-100] * user_prompt_len + tokenized_full_prompt["input_ids"][
                user_prompt_len:
...
def train(config: TrainerConfig) -> flytekit.directory.FlyteDirectory:
    ...
    os.environ["WANDB_API_KEY"] = flytekit.current_context().secrets.get(SECRET_GROUP, WANDB_API_SECRET_KEY)
    ...
    data = (
        ...
        else DatasetDict({"train": load_dataset(config.data_path, split=f"train[:{config.debug_train_data_size}]")})
    )
...
def save_to_hf_hub(
    ...
    api.upload_folder(repo_id=repo_id, folder_path=root, ignore_patterns=["flyte*", "models--*", "tmp*"])
    return str(repo_url)


@flytekit.task
def print_model_dir(model_dir: flytekit.directory.FlyteDirectory) -> str:
    print("Model directory is available at:", model_dir.remote_source)
    return model_dir.remote_source
...
def fine_tune(
    ...
    print_model_dir(model_dir=model_dir)
